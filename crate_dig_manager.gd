...
const MAX_DIGS = 3 # Example: Player can dig 3 times per 'night'
const SONG_CARD_SCENE= preload("res://scenes/song_card.tscn") # Needs access to the card scene!
const DatabaseScript = preload("res://scripts/song_database.gd")
...
func _ready():
	...
	var db_instance = DatabaseScript.new()
	if db_instance and db_instance.SONGS:
		master_database=db_instance.SONGS.duplicate()
		status_label.text= "Pick your crate! %d picks left" % digs_remaining
	else:
...
func _reveal_crate_contents(amount : int):
	var pool = master_database.duplicate()
	pool.shuffle()
